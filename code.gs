  ...
  try {
    ...
    let allQuestions = qaData.filter(q => forceString(q.id || q["รหัสอ้างอิง"] || q.subject || q["เรื่อง"]) !== "").map(q => {
      return {
        ...
        asker: isAdmin ? forceString(q.askername || q.asker || q["ผู้ถาม"] || q["ชื่อ"]) : "ปกปิดข้อมูล (เฉพาะ Admin)",
        askerDept: isAdmin ? forceString(q.askerdept || q["สังกัด"] || q["หน่วยงานผู้ถาม"]) : "ปกปิดข้อมูล",
        phone: isAdmin ? forceString(q.phone || q["เบอร์โทร"] || q["เบอร์โทรศัพท์"]) : "-",
        email: isAdmin ? forceString(q.email || q["อีเมล"]) : "-",
...
function submitQuestion(fd) {
  ...
  try {
    ...
    // แก้ไข: ตรวจสอบและเติมเครื่องหมาย ' นำหน้าเบอร์โทรศัพท์เพื่อป้องกันเลข 0 หายเมื่อบันทึกลง Google Sheets
    setVal(["phone", "เบอร์โทร", "โทรศัพท์"], forceString(fd.phone) ? "'" + forceString(fd.phone) : ""); 
...
function updateQuestionDept(id, newDept) {
  ...
  try {
    ...
    const colCat = getColIndex(sheet, ["Category", "หมวดหมู่", "ประเภท", "ประเภทคำถาม"]); // ✅ เพิ่มการหาคอลัมน์หมวดหมู่

    // 🔍 ค้นหาหมวดหมู่ (Category) ที่ถูกผูกไว้กับหน่วยงานใหม่ (newDept)
    let newCategory = "อื่นๆ"; 
    const categories = getSheetData("Categories");
    for (let i = 0; i < categories.length; i++) {
      let mapped = forceString(categories[i].mappeddepts || categories[i]["หน่วยงานผูกสิทธิ์"] || categories[i]["หน่วยงานรับผิดชอบ"] || categories[i]["หน่วยงาน"]);
      let deptsArray = mapped.split(',').map(d => d.trim());
      if (deptsArray.includes(forceString(newDept))) {
        newCategory = forceString(categories[i].name || categories[i].Name || categories[i]["ชื่อ"] || categories[i]["ประเภท"] || categories[i]["ประเภทคำถาม"]);
        break; // ค้นพบหมวดหมู่แรกที่ตรงกันให้หยุดค้นหาเพื่อความรวดเร็ว
      }
    }

    for (let i = 1; i < data.length; i++) {
      if (forceString(data[i][colId - 1]) === forceString(id)) {
        if (colTarget !== -1) sheet.getRange(i + 1, colTarget).setValue(newDept); // อัปเดตหน่วยงานเป้าหมาย
        if (colCat !== -1) sheet.getRange(i + 1, colCat).setValue(newCategory); // ✅ อัปเดตประเภทคำถามให้สอดคล้องกันทันที
        clearSystemCache();
        return JSON.stringify({ status: "success" });
      }
    }
...
function submitAnswer(id, answerTxt, fileData, fileName, staffName, ansByDepts) {
  ...
  try {
    ...
    const cSubject = getColIndex(sheet, ["Subject", "เรื่อง", "หัวข้อ"]); // เพิ่มสำหรับดึงชื่อเรื่องไปโชว์ใน LINE
    ...
    for (let i = 1; i < data.length; i++) {
      if (forceString(data[i][cId - 1]) === forceString(id)) {
        ...
        // ==========================================
        // 🟢 ส่งแจ้งเตือน LINE (เจ้าหน้าที่ตอบคำถามแล้ว)
        // ==========================================
        try {
...
// ==========================================
// 🟢 ระบบแจ้งเตือน LINE อัจฉริยะ (รองรับ Notify & Messaging API)
// ==========================================
// ⚠️ แนะนำ: หากระบบส่งครบ 200 ข้อความแล้ว LINE OA จะบล็อกการส่ง แนะนำให้เปลี่ยนมาใช้ Token ของ "LINE Notify" แทน (Token จะสั้นๆ) เพื่อส่งฟรีไม่จำกัดครับ
const LINE_TOKEN = "N+7KB2+9mLkBXq5LAaPK3m49nkEdPGOOHcXxmvdP9+4+6EibSQEgFVV+WXxwrSK8yY56yC2v7yYN37qQW+t1sRuOrk8Hle5gtN/b9fmzyGK2k2+gDw3sxKJvXwIP2zq5KBW91YRZbeeOh56/BuMiTwdB04t89/1O/w1cDnyilFU=";

function sendLineNotification(message) {
  if (!LINE_TOKEN || LINE_TOKEN === "") return;
  
  try {
    if (LINE_TOKEN.length > 50) {
      // 1. ส่งผ่าน LINE Official Account (Broadcast - แจ้งเตือนทุกคนที่ติดตามบอท)
      ...
      const options = {
        ...
        "muteHttpExceptions": true // อนุญาตให้แสดง Error จริงโดยไม่ทำให้ระบบล่ม
      };
      const response = UrlFetchApp.fetch(url, options);
      console.log("LINE OA Status: " + response.getResponseCode() + " | Response: " + response.getContentText());
      
    } else {
      // 2. ส่งผ่าน LINE Notify ปกติ (แจ้งเตือนเข้ากลุ่มหรือส่วนตัว)
      ...
      const response = UrlFetchApp.fetch(url, options);
      console.log("LINE Notify Status: " + response.getResponseCode() + " | Response: " + response.getContentText());
    }
  } catch(e) { 
    console.error("Critical LINE Error: " + e.message); 
  }
}
